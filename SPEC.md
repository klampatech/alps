...
**Smoke #20 caveats vs. smoke #18 (1:1 parity claim):** (1) wall-clock 5744s vs 3580s (1.6×); (2) orchestrator performed **~8 outer-loop iterations** vs smoke #18's 3 — codex shipped 9/9 stories but kept adding polish stories to prd.json on each retry pass; (3) `receipts.json` warning (`WARNING: no receipts.json found under tasks/`) — same warning was present in smoke #19 (pre-fix), suggesting a pre-existing orchestrator-level bug in the receipts-write path that is **NOT in this PR's scope** (smoke #18 also showed a receipts.json but wrote it from the bash Ralph subprocess's exit). Outer-loop churn is hypothesized to come from the Judge accepting then re-running (per `ImplementError::IncompleteStories` guard, `Judge ACCEPT → Plan → Implement → new prd.json → Judge ACCEPT again`). **Follow-up PR(s) planned:** investigate outer-loop convergence (likely an orchestrator-level issue) and the missing receipts.json (likely a pre-existing orchestrator bug).
