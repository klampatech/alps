...
1. **P1 — Surface tool exit code + stderr on every Ralph iteration.** Current behavior: Ralph's `run_codex_once` (and equivalent for claude/amp) catches the child's `Output` / `Status` and only logs a generic "iteration failed, retrying" line when exit code ≠ 0. The actual exit code, stderr tail, and any 429 / `quota_exceeded` / `rate_limit_error` markers from the LLM provider are swallowed. **Fix:** every iteration's `Result<Output, _>` should `elog!` the exit code + the last 1 KB of stderr verbatim. **Acceptance gate:** a unit test that drives `ralph::run` with a fake codex that exits 1 with `"429 rate_limit_error"` on stderr; assert the captured stderr tail lands in `<ralph_dir>/.ralph-stderr.log` (which already exists for the O_APPEND mirror). Cheap — single line change in `ralph.rs`. P1 because the next quota-burn incident will burn MORE quota if we can't see *why* it failed.
