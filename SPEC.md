...
9. **Orchestrator death on iteration-2 *implement.run*** — *promoted to active 2026-08-06, **PR [#9](https://github.com/klampatech/alps/pull/9)** surfaces it but does not fix it.* Smoke #10 (2026-08-06, Tier 4 notes-app, 8 stories) reached `[rejected]` with legitimate findings (port mismatch + CSS exclusivity violation), restarted with feedback, started the second Plan→Implement run, and the orchestrator process disappeared mid-`implement.run` *after* logging `[implement] invoking Ralph: tool=codex, max_iterations=20, stories=8` but *before* logging `[implement] done`. Codex (the child of ralph.sh, which is the child of alps) was orphaned to systemd and kept running. The smoke #7 instrumentation proved this is NOT a panic (the `ALPS_PANIC_LOG` file is empty) and NOT a §12 item 9 / §12 item 10 telemetry / wrapper bug (those are now fixed in PR #9). The orchestrator's first `implement.run` invocation completes cleanly; the death happens on the second-or-later invocation triggered by the reject→restart path. **Hypothesis:** the alps process is being killed by an external signal (SIGTERM, possibly from a herdr / Claude TUI / terminal-watcher timeout) that doesn't fire on the first invocation but does on subsequent ones. Fix candidates: (a) install a `signal-hook` SIGTERM handler that emits `[alps-diag] received SIGTERM` to the telemetry log so we can prove the killer — would catch the signal in milliseconds and write the backtrace to `ALPS_PANIC_LOG` even though it's not a panic; (b) investigate herdr's `'Terminated'` behavior across iterations (the pane's bash shows "Terminated" only on subsequent iterations, never the first); (c) check whether the orchestrator is OOM-killed by the second `read_artifacts` walk (smoke #10 picked up 71 artifacts vs 68 in smoke #8, both well under any meaningful threshold). The first iteration's full [rejected] path is verified working — this is a follow-up to make the second-and-later iterations robust.
