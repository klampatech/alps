...
#### Next-session ramp: structured-DoD follow-up

If a new session is pointed at this follow-up, it should be able to start work without re-discovering the context. Concrete ramp:

1. **Branch + worktree.** `cd ~/Development/alps && git checkout refactor/alps-ralph-rust-port` (4 commits ahead of `main`: `f4acb3f`, `74fe9f9`, `0880496`, …). All port + smoke #21 work is on this branch. The smoke wrapper + its test live in `/tmp/`, not in the repo, because the operator keeps `/tmp/alps-tier4-smoke-wrapper.sh` operator-side.
2. **Load skills in this order** (the ones that *will* fire the prompt, not just be on disk): `projects/alps` (overall architecture + invariants), `alps-ralph-port-context` (post-#21 ledger + diagnostic patterns), `alps-orchestrator-stderr-flush` (the `elog!` macro + the FD-2 buffer-drop story, which still explains why telemetry has its own file), `devops/herdr-iteration-workflow` (the `pane run` + `wait output` pattern for any re-smoke), `software-development/systematic-debugging` (4-phase: don't skip to a fix).
3. **Reproduce the regression cheaply first.** `cargo test -p alps-core --test <dod_test_name> -- --exact --nocapture` — find the existing `dod_runner` test, see if any of them already cover `pyproject.toml` / `package.json` resolution at the *deliverable* path. The skill says the detection walks `Implementation.deliverable_path`; smoke #18 (PR #3 commit `0d840c1`) was supposed to fix that walk but smoke #21 says it didn't. Start by reading `alps-core/src/judge.rs` `dod_runner.detect_project_type` and tracing which root it actually walks.
4. **Then** write a failing test that mirrors smoke #21's state (deliverable at `…/notes-21`, `pyproject.toml` + `package.json` in subdirs) and asserts the detector returns `ProjectType::Python` and `ProjectType::Node` rather than `Unknown`. RED first, GREEN second.
5. **Verify end-to-end.** Re-run a Tier-4 smoke in a herdr pane. The acceptance gate from the follow-up paragraph is the test for "done." Do not weaken the `IncompleteStories` guard or the `wrapper` / `CWD` fixes while doing this work; they are load-bearing.
6. **Don't re-smoke #21 unprompted.** Smoke #21 cost ~18 min of operator-side LLM spend. Investigation + unit regression test is enough to clear this follow-up. A re-smoke only if the regression test forces a new code path that the unit test can't cover.
