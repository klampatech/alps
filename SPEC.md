...
*Stability follow-ups (required before calling the Rust port fully production-stable; separate changes from the port PR):*

1. **P0 — wrapper stream separation.** Change `/tmp/alps-tier4-smoke-wrapper.sh` so process stderr and `ALPS_TELEMETRY_LOG` never target the same file. Keep `${STDERR_LOG}` for shell/child FD-2 and introduce a distinct `${TELEMETRY_LOG}` for `elog!` structured output. Update preservation/metadata output to retain both files. **Acceptance gate:** a short deterministic wrapper test emits uniquely numbered markers through both paths and proves each marker appears once, in per-stream order; the next Tier-4 smoke must be reconstructable without relying on filesystem mtimes to correct log ordering.

2. **P0 — outer-loop Plan→Implement story-contract investigation.** Smoke #20 pass 1 legitimately implemented the Plan's 9/9 stories and then received a Judge Reject. On pass 2, Plan generated a **fresh 10-story PRD**, while the prior Ralph implementation/history represented 9 stories; the new Ralph run then ended 0/10 and the completion guard correctly stopped the pipeline. Determine whether the tenth story is a valid decomposition of Judge feedback or drift caused by disagreement between the Plan prompt template and the Tier-4 smoke prompt's definition of a user story. Trace the exact `plan.json` → `prd.json` transformation across both passes, verify feedback/history propagation, and pin the intended invariant in a regression test. **Acceptance gate:** after a Reject, story IDs/counts are either preserved or changed through an explicit, test-covered reconciliation rule; a re-smoke reaches Review/Judge without a phantom 0/N reset.

These follow-ups serve ALPS's primary engineering goal: **continually reduce ambiguous state, nondeterministic diagnostics, and cross-stage contract drift until failures are explicit, reproducible, and safely contained.** Do not weaken the `IncompleteStories` guard to make the smoke green—the guard behaved correctly and remains load-bearing.
