...
|> **Status**: v0.8 — orchestrator working end-to-end for Python, Rust, Node, Go; reject path verified in production; ralph in-process Rust port on `main`; structured-DoD monorepo depth-1 walk + Tier-4 cwd fix + venv-aware Python Judge all shipped and smoke-verified; alps-on-alps smoke #26 ACCEPTED. **184/184 tests passing** (152 lib + 5 integration + 4 + 23 alps-binary). Tier-4 full-stack (FastAPI + Postgres + Vite/React + Alembic + bcrypt) verified end-to-end via smoke #26.
|> **Author**: Kyle + Evo
|> **Date**: 2026-07-26 (initial); 2026-07-27 (v0.1 implementation); 2026-08-03 (v0.7.2 — major architectural drift cleanup); 2026-08-11 (v0.8 — cwd-fix + ralph-rust-port + venv-aware Judge + Tier-4 smoke #26 ACCEPTED, this revision)
...
| 2026-08-10 | `9b2d120` | **CI-safe monorepo cwd regression tests** — added `cfg(target_os = "linux")` + `pkg_available()` skip guard to the two new Tier-4 monorepo integration tests so CI on `ubuntu-latest` skips them when `python3 -c "import pytest"` fails, while still running locally on dev hosts with pytest installed. 0 new failures on CI; 179 → 180 tests locally. **Surfaced by smoke #23 (2026-08-10):** CI on PR #17's branch failed because the new monorepo-cwd tests required a python3+pytest stack that the default ubuntu-latest runner image didn't ship. **Pattern:** "tests that depend on a system package should be skippable on CI without the package, not just locally." |
| 2026-08-10 | `9b2d120` (PR #18) | **PR #18 MERGED** — `Merge pull request #18 from klampatech/fix/ci-safe-test-for-monorepo-cwd` (klampatech merge, 1m28s CI). 180/180 tests, build+test `pass 1m28s`. The §12 P1 cwd fix is now on `main`. The Judge's structured-DoD stage runs pytest from the matched subdir, not the deliverable root. Unblocks the Python monorepo leg of Tier-4 smokes. |
| 2026-08-10 | (smoke #23) | **Tier-4 smoke ACCEPTED** — first clean Tier-4 Judge ACCEPT post-PR-#18. Iter 1: 8/8 stories, 9 commits, 12/12 review assertions, Judge `claude-opus-4` ACCEPTED, 6-min wall clock, 0 SIGTERM markers. Iter 1's structured DoD ran `[judge:structured] running: /tmp/.../backend/.venv/bin/python -m pytest -q` → exit Some(4) (sqlalchemy missing). After the cwd fix from `6c1fadf`, the Judge correctly cd'd into `backend/` — surfaced the venv bug (separately) instead of the cwd bug. **Two-bug isolation confirmed:** cwd vs venv are independent, not the same regression. |
| 2026-08-10 | (smoke #24-baseline) | **Tier-4 cwd-fix ABL smoke** — re-fired smoke #22-tier4's prompt against the pre-fix `6ae0915` binary to prove the cwd regression is reproducible. Iter 1: pytest exit Some(2) (`ModuleNotFoundError: No module named 'sqlalchemy'`), confirms the cwd bug reproduces deterministically without `6c1fadf`. Combined with smoke #23 (post-fix) and smoke #22-tier4 (pre-fix) this gives the 3-point evidence chain needed to claim a true root-cause fix. |
| 2026-08-10 | (smoke #25) | **Tier-4 venv-fix pre-smoke** — first smoke against the cwd fix on `main`. Iter 1: cwd fix verified (`test_root: .../backend` in log), but pytest still fails with exit Some(4) because the Judge invokes `python3` (PATH → `/usr/bin/python3`) which lacks sqlalchemy. This isolated the venv bug from the cwd bug and surfaced §12 P4. **Note:** smoke #25 was terminated by herdr-wait timeout at 20:23 (US-003 implementation committed at `565763a` but 29k lines of in-progress work lost). New smoke-failure shape distinct from smoke #23's pane-not-found — `wait-exit-code=1` + SIGTERM, recommended fix: extend herdr-wait timeout budget for Tier-4 smokes. |
| 2026-08-11 | `d904bca`, `10b0fb6`, `278609e` (PR #19) | **§12 P4 — Judge venv-aware test invocation (Option C).** The orchestrator's Judge now prefers `<test_root>/.venv/bin/python`, then `<test_root>/venv/bin/python`, before falling back to system `python3` — a single-file change to `alps-core/src/judge.rs:1009` (signature changed to take `test_root: &Path`, returns `(Cow<'static,str>, Vec<Cow<'static,str>>)`). Tier-4 Python monorepos with `.venv/` now get Judge pytest from the right interpreter; Tier-1 single-dir Python, Tier-2/3 (Go/Node), and Python-without-venv all unchanged. **Tests:** 180 → 183 (+ 3 new pure-data tests: `test_command_for_python_uses_venv_python_when_venv_exists`, `test_command_for_python_prefers_dotvenv_over_venv`, `test_command_for_python_falls_back_to_python3_when_no_venv`). Plus the cheap live-resolution smoke `test_command_for_python_resolves_real_tier4_venv` that drives `test_command_for` against the smoke #25 leftover deliverable. **Verified end-to-end by smoke #26** (next row). |
| 2026-08-11 | (smoke #26) | **Tier-4 full-stack smoke #26 ACCEPTED ✅.** First alps-on-alps Tier-4 run to clear the Judge end-to-end with the §12 P4 venv fix in place. 4 outer iterations: iter 1 reject (missing `tmp_listing.txt`), iter 2 reject (port 5174 vs spec 5173, no PIL dim-verification log), iter 3 reject (`hash_password` bytes input + persistence proof unverifiable), **iter 4 ACCEPTED**. Final state: 19/19 stories passed, 52 commits, 63 artifacts, 1 Ralph iteration (1332s elapsed), 13/13 review assertions passed, 0 critical findings, Judge `claude-opus-4` ACCEPTED. **`[judge:structured] running: /tmp/alps-tier4-notes-26/backend/.venv/bin/python -m pytest -q` → PASS** on every iteration. Deliverable: FastAPI + async Postgres + JWT (bcrypt) + Alembic + Vite + React + TS frontend at `/tmp/alps-tier4-notes-26/`, 18 captured artifacts including 5 Playwright screenshots at 1280x800 (PIL-verified). Real `receipts.json` written; auto-committed final state. **§12 P4 acceptance gate: MET.** |
| 2026-08-11 | `a20c586` (PR #19) | **PR #19 MERGED** — `Merge pull request #19 from klampatech/docs/spec-p4-venv-aware-judge` (klampatech squash, 15:21:37 UTC, 1m1s CI green). 183 → 184 tests on `main` (the +1 is `test_command_for_python_resolves_real_tier4_venv` which landed in `278609e` after §12 was drafted). The venv-aware Judge is now in production. |
...
- [x] ~~**Mock-agent happy-path test**~~ — landed 2026-08-10 (`3211bed`). `drive_passes_first_try` family (3 variants) + `drive_rejects_twice_then_passes_accumulates_agents_md`. 175 → 176 tests.
...
- [x] **Tier 4** — Full-stack notes app (FastAPI + async Postgres + JWT/bcrypt + Alembic + Vite + React + TS) — **verified 2026-08-11 via smoke #26** (herdr `wF6:p1`), 4 outer iterations, 19/19 stories, 13/13 review assertions, 0 critical findings, Judge `claude-opus-4` ACCEPTED. 18 captured artifacts including 5 Playwright screenshots (PIL-verified 1280x800). Requires the §12 P4 venv-aware Judge fix from PR #19.
- [ ] **Tier 5** — Rust full-stack notes service (Cargo workspace with `core` + `api` + `cli` crates, Axum + sqlx + Postgres + JWT + argon2) — **spec written 2026-08-11** (`docs/tier5-spec.md`), smoke runner in `scripts/tier5-smoke.sh`, smoke not yet fired. Tier 5 is the first multi-crate Rust workspace smoke; stresses the structured-DoD monorepo detector on a Rust shape (one root `Cargo.toml` + nested member crates) instead of Tier 4's Python `backend/` + `frontend/` shape. Codifies the smoke-#26 filesystem/telemetry monitor pattern as the default runner, structurally closing §12 P7's `herdr wait` failure shape.
...
This is the live roadmap as of 2026-08-11. Items in **bold** are
...
3. ~~**Mock-agent happy-path test**~~ — **SHIPPED 2026-08-10** (`3211bed`). The `drive_passes_first_try` family (basic pass-on-first-call, receipts.json-on-disk, AGENTS.md propagation) plus the multi-iteration `drive_rejects_twice_then_passes_accumulates_agents_md` pin both the single-iter happy path AND the cross-iteration accumulation contract. Test count 175 → 176. The original "multi-iteration happy-path (3 Plan→Implement→Review→Judge round-trips, all PASS)" framing was structurally wrong — `drive()` terminates on first `Judgment::Pass`, so a multi-iter happy path is impossible by design. What multi-iter actually needs is the accumulation test (ralph patterns + feedback must reach the next Plan call), which is now in place.
...
**P4 verification (2026-08-11, smoke #26):** see §0 changelog row "smoke #26 ACCEPTED". The venv fix is now in production and confirmed against the real Tier-4 deliverable.

5. **P5 — `--budget <hms>` CLI flag for the outer loop (rate-limit/quota awareness).** *Promoted from §12 P2 (rate-limit series) — open as of 2026-08-09.* Surface: alps fires `claude` / `codex` / `amp` calls and the operator has no way to say "abort after N minutes, don't care how many stories pass." When the operator hits a quota wall mid-smoke, the orchestrator happily retries within `max_iterations` until it hits `IncompleteStories` and exits non-zero — by which point several more LLM calls have been billed. **Fix:** new `--budget <hms>` CLI flag on `alps run`; wraps `loop_.rs::drive` in `tokio::time::Instant::now() + budget` and on each iteration checks `Instant::now() >= deadline`, returning `AlpsError::BudgetExceeded` if crossed. **Acceptance gate:** a unit test that drives `drive()` with `Instant::now() + 100ms` budget and a scripted Judge that takes 200ms per call; assert `drive()` returns `Err(BudgetExceeded)` after the budget elapses and writes no `receipts.json` (matches the `AlpsError::Implement(IncompleteStories)` pattern — clean Err exit, no orphan artifacts). P5 because the next quota wall is also the moment the operator most wants the abort.

6. **P6 — Tool cooldown / fallback on persistent 429.** *Promoted from §12 P3 (rate-limit series) — open as of 2026-08-09.* Current behavior: when codex 429s three iterations in a row, Ralph just iterates 4, 5, 6... burning more quota for the same outcome. **Fix:** a per-tool cooldown state in `RalphConfig`: if a tool returned 429 in the last N iterations (configurable, default 3), switch to the next tool declared in `RalphConfig::tool_fallback_chain` (e.g. `codex → claude → amp`). Surface a clear `[ralph] switching tool: codex → claude after 3 consecutive 429s` line in the telemetry log so the operator can see it happen. **Acceptance gate:** a unit test that drives `ralph::run` with a fake codex that 429s 4 times in a row and a fake claude that succeeds; assert the deliverable ships via claude with the fallback line in the log. P6 because it touches the tool-dispatch hot path, but it's the only change that *actively prevents* quota burn rather than just detecting it.

7. **P7 — Tier-4 smoke wrapper herdr-wait timeout (new failure shape).** *Surfaced 2026-08-10 by smoke #25; deferred 2026-08-11 in favor of Tier 5.* Symptom: the herdr-wait timeout fires before the Tier-4 smoke finishes, killing the orchestrator mid-implementation. The smoke wrapper is fine; the operator-side `herdr wait output <pane> --timeout 3600000` budget is too tight for Tier-4 (smoke #26 took ~1h45m wall clock including 4 outer iterations). **Diagnostic fingerprint:** `wait-exit-code=1` + stderr-final-line `[alps-signal] received SIGTERM` + a long stderr log (29k lines for smoke #25, US-003 implementation in flight at termination). Distinct from smoke #23's `pane-not-found` (wait lost the pane) and the 4 max_iter shapes (smoke completes but Judge rejects). **Fix path:** the smoke-#26 win was substituting the `herdr wait` pattern entirely with a filesystem/telemetry monitor loop (no `herdr` command in the loop). That pattern is now codified in `scripts/tier5-smoke.sh` (smoke runner, in-repo) — Tier-5+ smokes use the runner by default, making the smoke-25 failure shape structurally impossible. **No code change to alps itself.** P7 is **DEFERRED, not closed** — if Tier 5 reveals the filesystem/telemetry monitor pattern is brittle (e.g., the wrapper itself dies on certain shell signals, or the monitor loop misses a convergence signal), P7 re-opens with a more substantive fix. See `docs/tier5-spec.md` §"Why P7 is being deferred" for the full deferral rationale.

*Acceptance contract for all three (P5, P6, P7):* none of them weaken the `IncompleteStories` guard, the tool-CWD fix from `74fe9f9`, the wrapper stream separation, the structured-DoD monorepo walk, the §12 P1 cwd fix (`6c1fadf`), or the §12 P4 venv-aware Judge fix (`10b0fb6`). All three are additive — strictly more visibility / more safety, no regressions to existing load-bearing behavior.



### Recently completed (just shipped)

- ~~**§12 P4 — Judge venv-aware Python test invocation (Option C)**~~ — landed 2026-08-11, **PR [#19](https://github.com/klampatech/alps/pull/19)** commits `d904bca` + `10b0fb6` + `278609e` (squashed at `a20c586`, 1m1s CI green). Single-file change to `alps-core/src/judge.rs:1009`; signature now `(project_type, test_root) → (Cow<'static,str>, Vec<Cow<'static,str>>)`. Verifying test: `test_command_for_python_resolves_real_tier4_venv` against the real smoke #25 deliverable. Test count 180 → 184. End-to-end verified by smoke #26 (Tier-4 ACCEPT, 4 outer iterations, 19/19 stories). The cwd-vs-venv bug isolation (smokes #22 → #23 → #24-baseline → #25 → #26) is documented in the §0 changelog.
- ~~**§12 P1 — Judge structured-DoD cwd fix**~~ — landed 2026-08-10, **PR [#17](https://github.com/klampatech/alps/pull/17) / #18** commits `6c1fadf` + `9b2d120` (squashed at `cc800f9`, 1m28s CI green). `detect_project_type(dir)` now returns `(ProjectType, PathBuf)` and the Judge's `run_cmd_with_timeout` uses the matched subdir as cwd. +2 monorepo integration tests (skippable when `python3 -c "import pytest"` fails, so CI-safe on ubuntu-latest). 177 → 180 tests. Verifying smokes: #23 ACCEPT, #24-baseline reproduces pre-fix failure (3-point evidence chain), #25 surfaces the venv bug (proves the fixes are independent).
- ~~**§12 P2 (rate-limit series) — P1 surface tool exit code**~~ — landed 2026-08-10, commit `d60fa33`. All three tool branches (codex / claude / amp) capture `ExitStatus` and log `[ralph] <tool> exited N on iteration I/M (stderr mirror at <ralph_dir>/.ralph-stderr.log)`. The stderr mirror was already working; now the exit code is visible. Acceptance test `quota_exceeded_stderr_lands_in_ralph_stderr_log_and_loop_continues` pins both contracts. 176 → 177 tests.
- ~~**§12 item 3 — Mock-agent happy-path test**~~ — landed 2026-08-10, commit `3211bed`. `drive_passes_first_try` family (3 variants) + `drive_rejects_twice_then_passes_accumulates_agents_md`. 175 → 176 tests.
