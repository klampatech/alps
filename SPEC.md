...
1. **Deliverable-outside-workdir gap** *(surfaced 2026-07-30 by the herdr-watch CRUD smoke)* — when the prompt specifies a target path outside `--workdir` (e.g. "Build at `/tmp/foo-2/`"), codex writes there as told, but `read_artifacts` only walks `tasks/.../implementation/ralph/`. Hermes then has empty source files and rejects everything even when `pytest -q` is green at the target. Smoke 1 of the CRUD pair worked *by accident* (codex mirrored into ralph cwd per its US-001 progress pattern); smoke 2 hit the gap deterministically and was killed after iter 2. **A (prompt-side) shipped 2026-07-30 via skill update** — alps smoke recipe + herdr-iteration-workflow now remind the operator to write the deliverable *inside* the workdir. B + C remain open for the structural fix. Three fix approaches, in increasing order of correctness vs cost:

   - ~~**A. Prompt-side**~~ *(shipped 2026-07-30)* — recipe change in `alps-core/SKILL.md` "Smoke test recipe" + prompt-side reminder in the wrapper-script pattern. Closes the common case where the operator forgets; doesn't help when the user actively wants work outside workdir.
   ...
   **Next move:** B is the structural fix and is the candidate for the next session.
