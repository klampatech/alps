...
7. **P7 — Tier-4 smoke wrapper herdr-wait timeout (new failure shape).** *Surfaced 2026-08-10 by smoke #25.* Symptom: the herdr-wait timeout fires before the Tier-4 smoke finishes, killing the orchestrator mid-implementation. The smoke wrapper is fine; the operator-side `herdr wait output <pane> --timeout 3600000` budget is too tight for Tier-4 (smoke #26 took ~1h45m wall clock including 4 outer iterations). **Diagnostic fingerprint:** `wait-exit-code=1` + stderr-final-line `[alps-signal] received SIGTERM` + a long stderr log (29k lines for smoke #25, US-003 implementation in flight at termination). Distinct from smoke #23's `pane-not-found` (wait lost the pane) and the 4 max_iter shapes (smoke completes but Judge rejects). **Fix:** bump the smoke-#26 wrapper's `herdr wait` timeout to `7200000` (2h) or split it into per-iteration waits. **No code change to alps itself — operator-side wrapper config.** Document the new failure shape so future smokes don't get misdiagnosed as codex/runner issues.
