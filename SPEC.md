...
*Follow-ups (separated from this PR):*
1. **Wrapper fix:** change `/tmp/alps-tier4-smoke-wrapper.sh` to capture stderr and telemetry in **separate files** (e.g., `${STDERR_LOG}` for stderr, `${TELEMETRY_LOG}` for the orchestrator's structured output) so future smokes have reliable line ordering.
2. **Outer-loop investigation:** smoke #20's implement-completion guard correctly caught `0/10 stories_passed`, but the orchestrator **should not have attempted a re-implement** if Ralph's first attempt passed 9/9. The Plan agent is producing PRDs that are 1 story longer than what Ralph implements, suggesting a Plan/Implement story-count mismatch. Needs investigation of the Plan prompt template vs the smoke prompt's story definition.
