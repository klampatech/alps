...
**Smoke #20 caveats vs. smoke #18 (1:1 parity claim):** (1) wall-clock 5744s vs 3580s (1.6×); (2) orchestrator performed **~8 outer-loop iterations** vs smoke #18's 3 — codex shipped 9/9 stories but kept adding polish stories to prd.json on each retry pass; (3) **`receipts.json` is NOT being written** to `tasks/<id>/receipts.json` — confirmed missing in both smoke #19 (pre-fix) and smoke #20 (post-fix) preserved dirs, while smoke #17 + #18 (bash Ralph) both wrote receipts.json (707 and 796 bytes respectively). The smoke wrapper's "WARNING: no receipts.json found under tasks/" message is a real symptom, not a transient false positive.

**Investigation findings (post-smoke #20, 2026-08-08):**

*Receipts.json bug.* `[done] accepted` marker is logged 31 times across smoke #20's run, but `tasks/<id>/receipts.json` is never created. Grep confirms `[done] accepted` is emitted from exactly one location (`alps-core/src/loop_.rs:162`) inside the `Ok(done)` branch of `run_iteration`'s `match task.judge(judgment)`. The code path is: `task.judge(judgment)` returns `Ok(done)` → `persist_task(&done, workspace).map_err(AlpsError::Persistence)?` → `elog!("[done] accepted")` → `Ok(done)`. The `?` propagates errors, so if `write_receipts` failed we'd see `error: ...` and the CLI would exit. Since `[done] accepted` IS logged, `persist_task` IS succeeding. Yet no file on disk. The most likely cause: `write_receipts` is being called, the `std::fs::write` at `persistence.rs:115` is truncating an existing file, but the file is being deleted or rolled back by a different process between iterations. (Hypothesis: some leftover from a prior task is preventing the new write? Or `workspace.root` was moved mid-run?) **Needs follow-up:** a focused repro with `--prompt-file` to isolate whether `write_receipts` is failing silently or being called with a wrong path. Add a test that exercises the `Ok(done)` branch and verifies the file lands.

*Outer-loop churn.* Smoke #20 ran 81 `[plan] running` invocations, 54 `[judge] running` invocations, 31 `[done] accepted` markers, 13 `[rejected]` markers, and the orchestrator's final state was the implement-completion guard correctly catching `0/10 stories_passed` (Ralph returned `completed: true` from `.ralph-result.json` but the prd had been rewritten with `passes: false` on every story — likely a codex behavior where it adds polish stories then fails to mark them passing in time). Two real issues here: (a) **`task.judge()` returning `Ok(done)` is NOT terminating the orchestrator** — 31 accepts should have produced 31 receipts.json and exited, but the orchestrator kept running; (b) **dual-writer log race**: `elog!` writes both to stderr (via shell `2>> ${STDERR_LOG}`) and directly to `${STDERR_LOG}` via `OpenOptions::append(true)`. Two writers, one file, different open flags — POSIX `O_APPEND` guarantees intra-writer atomicity but **not** inter-writer ordering. The result is `[done] accepted` markers appear at arbitrary byte positions in the log, making line-number-based reasoning unreliable. **Fix needed:** make the wrapper capture stderr and telemetry in **separate files** to eliminate the dual-writer race, then re-run smoke #21 to diagnose the orchestrator termination issue with a clean log.

**Follow-up PR scope (separate from this Rust Ralph port):**
1. **Receipts.json fix**: add a unit test that drives `Task<Done>` and verifies `receipts.json` lands on disk; investigate why it's missing despite the `[done] accepted` marker firing.
2. **Wrapper log-race fix**: separate stderr and telemetry streams in the smoke wrapper to give reliable line ordering for future diagnostics.
3. **Orchestrator termination**: investigate why `run_iteration` returns `Ok(done)` 31 times in smoke #20 without exiting — hypothesis is the binary is reading a stale or shared `loop_.rs` (binary was rebuilt mid-smoke? unlikely); needs a debug build with extra tracing to confirm.
